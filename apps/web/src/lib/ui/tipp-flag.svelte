<script lang="ts">
    ...
    let { label = "Joker-Tipp" }: Props = $props();
</script>
...
<Popover.Root>
    ...
    <Popover.Portal>
        <Popover.Content
            side="top"
            sideOffset={4}
            collisionPadding={{ top: 56 }}
            class="relative z-50 rounded bg-inverted px-2 py-0.5 text-xs whitespace-nowrap text-inverted"
        >
            {label}
            <Popover.Arrow class="text-(--background-color-inverted)" />
        </Popover.Content>
    </Popover.Portal>
</Popover.Root>
