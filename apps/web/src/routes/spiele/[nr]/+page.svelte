<script lang="ts">
    ...
    const sortedTips = $derived.by(() => {
        ...
        return [...tips].sort((a, b) => {
            if (sortCol === "name") return factor * a.name.localeCompare(b.name, "de");
            if (sortCol === "points") {
    ...
    const meta = $derived.by(() => {
        ...
        if (matchData.totalPoints !== null) parts.push(`${matchData.totalPoints} Pkt`);
        return parts.join(" · ");
    });
...
<div class="mx-auto w-full max-w-5xl py-8">
    <div class="xs:px-0 mb-6 flex flex-col items-center gap-2 px-4">
        <SpieleSelect rounds={data.roundsWithMatches} currentNr={matchData.nr} />
        {#if meta}
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-input border-b text-left">
                    <th class={thClass}>
                        <button onclick={() => handleSort("name")} class={sortBtnClass}>
                            Spieler
                            {#if sortCol !== "name"}
                                <ArrowUpDownIcon size={12} class="text-subtle/50" />
                            {:else if sortDir === "asc"}
            ...
            <tbody>
                {#each sortedTips as tip (tip.userId)}
                    <tr class="border-input border-b last:border-b-0">
                        ...
                        {#if matchData.tipsPublished}
                            <td class={cn(tdClass, "relative px-6 text-center")}>
                                {tip.tip ?? "–"}
                                {#if tip.joker}
                                    <TippFlag />
                                {/if}
                            </td>
                            <td class={cn(tdClass, "text-center")}>{tip.points ?? "–"}</td>
                        {/if}
                    </tr>
                {/each}
            </tbody>
