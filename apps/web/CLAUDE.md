...
- `lib/` — Server-only utilities (`*.server.ts`): `session`, `db`, `championship` (incl. `getRuleset`), `cookies`, `lock`, `ranking`, `round` (round-rule apply/revert — `applyRoundRule`, called by both the "Abgeschlossen" toggle and anything else that can change what a round rule computes), plus the per-view query modules `archiv`, `spiele`, `spieler`, `extra-questions`. Isomorphic: `context.ts`, `color-scheme.ts`, `utils.ts`. Kept as the one place showing every DB touchpoint — query modules stay here even when a single route uses them; the TOTP flow is not a query module and lives beside `/login`
