...
export async function loader({ context, params }: Route.LoaderArgs) {
  ...
  if (!championship || !Number.isInteger(nr)) {
    return { championshipName: championship?.name ?? null, match: null, ranking: [], rounds: [] };
  }

  const [match, ranking, rounds] = await Promise.all([
    getMatch(championship.id, nr),
    getRanking(championship.id),
    getRounds(championship.id),
  ]);

  return { championshipName: championship.name, match, ranking, rounds };
}
