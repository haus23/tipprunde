...
export function AppToastRegion() {
  return (
    <ToastRegion
      queue={toastQueue}
      className="focus-visible:ring-focus fixed right-4 bottom-4 z-50 flex flex-col-reverse gap-2 outline-none focus-visible:ring-2"
    >
      {({ toast }) => (
        <Toast
          toast={toast}
        ...
        >
          <ToastContent className="flex min-w-0 flex-1 flex-col gap-0.5">
            ...
            {toast.content.description && (
              <Text slot="description" className="text-xs opacity-80">
                {toast.content.description}
              </Text>
            )}
          </ToastContent>
          <Button
            ...
            className="flex size-8 shrink-0 cursor-default items-center justify-center rounded-md opacity-70 transition-opacity outline-none hover:opacity-100 focus-visible:ring-2 focus-visible:ring-[white]"
          >
            <XIcon size={16} />
          </Button>
        </Toast>
